...
export default class NPC extends Phaser.Physics.Arcade.Sprite {
  ...
  update() {
    ...
    if (this.move === "left") {
      ...
      if (isIdle) this.anims.play("hai_left", true)
      isIdle = false
    }
    if (this.move === "right") {
      this.body.setVelocityX(this.speed)
      if (isIdle) this.anims.play("hai_right", true)
      isIdle = false
    }

    if (this.move === "up") {
      body.setVelocityY(-this.speed)
      if (isIdle) this.anims.play("hai_up", true)
      isIdle = false
    }
    if (this.move === "down") {
      body.setVelocityY(this.speed)
      if (isIdle) this.anims.play("hai_down", true)
      isIdle = false
    }

    if (isIdle) {
      this.anims.play("hai_idle", true)
    }
  }
